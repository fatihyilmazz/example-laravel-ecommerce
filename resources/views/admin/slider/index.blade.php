...
@php
    ...
    $subHeaderBreadcrumbs = [['name' => __('text.slider.name_plural'), 'url' => route('admin.sliders.index'),'is_active' => true]];
@endphp
                ...
                <div class="kt-portlet__head-actions">
                    <div class="btn-group show">
                        ...
                        <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(-52px, 38px, 0px);">
                            @can('create-sliders')
                                <a href="{{ route('admin.sliders.create') }}" class="dropdown-item" ><i class="fa fa-plus-circle"></i>{{ __('text.slider.add_new') }}</a>
                            @endcan

{{--                            @can('view-sliders-histories')--}}
{{--                                <a href="{{ route('admin.sliders.histories') }}" class="dropdown-item" ><i class="fa fa-history"></i>{{ trans_choice('text.common.transaction_history', 2) }}</a>--}}
{{--                            @endcan--}}
        ...
        {{ html()->form('GET', route('admin.sliders.index'))->class(['filter-form'])->open() }}
        ...
        <div class="kt-portlet__body">
                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group row">
                                <div class="col-lg-12">
                                    <label for="slider_type_id" class="col-form-label">{{ __('text.slider_type.name_singular') }}</label>
                                    <select name="slider_type_id" id="slider_type_id" class="form-control select2 d-block w-100 @error('slider_type_id') is-invalid @enderror()">
                                        ...
                                        @foreach($sliderTypes as $key => $value)
                                            <option value="{{ $key }}" {{ (request()->get('slider_type_id') == $key ? 'selected' : null) }}>
                            ...
                            <div class="form-group row">
                                <div class="col-lg-6">
                                    <label for="started_at" class="col-form-label">{{ __('text.common.start_at') }}</label>
                                    {{ html()->text('started_at')
                                        ->class(['form-control date-range-single-picker-started-at', 'is-invalid' => $errors->has('started_at')])
                                        ->value(request()->get('started_at'))
                                        ->attributes(['autocomplete' => 'off']) }}
                                ...
                                <div class="col-lg-6">
                                    <label for="end_at" class="col-form-label">{{ __('text.common.end_at') }}</label>
                                    {{ html()->text('end_at')
                                        ->class(['form-control date-range-single-picker-end-at', 'is-invalid' => $errors->has('end_at')])
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-lg-6">
                                    <label for="is_published" class="col-form-label">{{ __('text.crud.status') }}</label>
                                    {{ html()->select('is_published')
                                        ->class(['form-control select2 d-block w-100', 'is-invalid' => $errors->has('is_published')])
                                        ->attributes(['autocomplete' => 'off'])
                        ...
                        <div class="modal-footer">
                            <a href="{{ route('admin.sliders.index') }}" class="btn btn-danger"><i class="fa fa-eraser"></i>{{ __('text.crud.clear') }}</a>
            ...
            <table class="table table-striped- table-bordered table-hover table-checkable responsive no-wrap" id="datatable">
                ...
                <tr>
                    ...
                    <th>{{ __('text.slider_type.name_singular') }}</th>
                    <th>{{ __('text.common.start_at') }}</th>
                    <th>{{ __('text.common.end_at') }}</th>
                    <th class="status-column">{{ __('text.crud.status') }}</th>
                    @canany(['view-sliders', 'update-sliders', 'delete-sliders', 'view-sliders-histories'])
                ...
                @foreach($sliders as $slider)
                    <tr class="table-row-id-{{ $slider->id }}">
                        <td>{{ $slider->id }}</td>
                        <td>{{ $slider->name }}</td>
                        <td>{{ __('text.slider_type.'. $slider->sliderType->translation_key) }}</td>
                        <td>{{ $slider->started_at->format('d.m.Y H:i') }}</td>
                        <td>{{ $slider->end_at->format('d.m.Y H:i') }}</td>
                        <td>
                            {!! $slider->is_published
                        ...
                        @canany(['view-sliders', 'update-sliders', 'delete-sliders', 'view-sliders-histories'])
                            <td>
                                @can('update-sliders')
                                    <a href="{{ $slider->editLink() }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{ __('text.crud.edit') }}"><i class="fa fa-edit"></i></a>
                                @elsecan('view-sliders')
                                    <a href="{{ $slider->editLink() }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{ __('text.crud.edit') }}"><i class="fa fa-eye"></i></a>
                                @endcan

    {{--                            @can('view-sliders-histories')--}}
    {{--                                <a href="{{ $slider->transactionHistoriesLink() }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{ __('text.crud.edit') }}"><i class="fa fa-history"></i></a>--}}
    {{--                            @endcan--}}

                                @can('delete-sliders')
                                    <button class="btn btn-sm btn-clean btn-icon btn-icon-md" type="button" onclick="confirmToDelete('{{ $slider->deleteLink() }}', {{ $slider->id }})" title="{{ __('text.crud.delete') }}">
                                        <i class="fa fa-trash" id="spinner-button-{{ $slider->id }}"></i>
                                    </button>
                                @endcan
                            </td>
                        @endcanany
