...
@php
    ...
    $subHeaderBreadcrumbs = [['name' => __('text.country.name_plural'), 'url' => route('admin.countries.index'),'is_active' => true]];
@endphp
                    ...
                    <div class="btn-group show">
                        ...
                        <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(-52px, 38px, 0px);">
                            @can('create-countries')
                                <a href="{{ route('admin.countries.create') }}" class="dropdown-item" ><i class="fa fa-plus-circle"></i>{{ __('text.country.add_new') }}</a>
                            @endcan

{{--                            @can('view-countries-histories')--}}
{{--                                <a href="{{ route('admin.countries.histories') }}" class="dropdown-item" ><i class="fa fa-history"></i>{{ trans_choice('text.common.transaction_history', 2) }}</a>--}}
{{--                            @endcan--}}
        ...
        {{ html()->form('GET', route('admin.countries.index'))->class(['filter-form'])->open() }}
        ...
        <div class="kt-portlet__body">
                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <a href="{{ route('admin.countries.index') }}" class="btn btn-danger"><i class="fa fa-eraser"></i>{{ __('text.crud.clear') }}</a>
            ...
            <table class="table table-striped- table-bordered table-hover table-checkable responsive no-wrap" id="datatable">
                ...
                <tr>
                    ...
                    @canany(['view-countries', 'update-countries', 'delete-countries', 'view-countries-histories'])
                ...
                @foreach($countries as $country)
                    <tr class="table-row-id-{{ $country->id }}">
                        <td>{{ $country->id }}</td>
                        <td>{{ $country->translationForCurrentLocale->name }}</td>
                        <td>
                            {!! $country->is_active
                        ...
                        @canany(['view-countries', 'update-countries', 'delete-countries', 'view-countries-histories'])
                            <td>
                                @can('update-countries')
                                    <a href="{{ $country->editLink() }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{ __('text.crud.edit') }}"><i class="fa fa-edit"></i></a>
                                @elsecan('view-countries')
                                    <a href="{{ $country->editLink() }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{ __('text.crud.edit') }}"><i class="fa fa-eye"></i></a>
                                @endcan

    {{--                            @can('view-countries-histories')--}}
    {{--                                <a href="{{ $country->transactionHistoriesLink() }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{ __('text.crud.edit') }}"><i class="fa fa-history"></i></a>--}}
    {{--                            @endcan--}}

                                @can('delete-countries')
                                    <button class="btn btn-sm btn-clean btn-icon btn-icon-md" type="button" onclick="confirmToDelete('{{ $country->deleteLink() }}', {{ $country->id }})" title="{{ __('text.crud.delete') }}">
                                        <i class="fa fa-trash" id="spinner-button-{{ $country->id }}"></i>
                                    </button>
                                @endcan
                            </td>
                        @endcanany
