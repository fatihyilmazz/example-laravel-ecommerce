...
@php
    ...
    $subHeaderBreadcrumbs = [['name' => __('text.currency.name_plural'), 'url' => route('admin.currencies.index'),'is_active' => true]];
@endphp
                    ...
                    <div class="btn-group show">
                        ...
                        <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(-52px, 38px, 0px);">
                            @can('create-currencies')
                                <a href="{{ route('admin.currencies.create') }}" class="dropdown-item" ><i class="fa fa-plus-circle"></i>{{ __('text.currency.add_new') }}</a>
                            @endcan

{{--                            @can('view-currencies-histories')--}}
{{--                                <a href="{{ route('admin.currencies.histories') }}" class="dropdown-item" ><i class="fa fa-history"></i>{{ trans_choice('text.common.transaction_history', 2) }}</a>--}}
{{--                            @endcan--}}
        ...
        {{ html()->form('GET', route('admin.currencies.index'))->class(['filter-form'])->open() }}
        ...
        <div class="kt-portlet__body">
                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <a href="{{ route('admin.currencies.index') }}" class="btn btn-danger"><i class="fa fa-eraser"></i>{{ __('text.crud.clear') }}</a>
            ...
            <table class="table table-striped- table-bordered table-hover table-checkable responsive no-wrap" id="datatable">
                ...
                <tr>
                    ...
                    <th>{{ __('text.currency.use_constant') }}</th>
                    <th>{{ __('text.currency.constant_price') }}</th>
                    <th class="status-column">{{ __('text.crud.status') }}</th>
                    @canany(['view-currencies', 'update-currencies', 'delete-currencies', 'view-currencies-histories'])
                ...
                @foreach($currencies as $currency)
                    <tr class="table-row-id-{{ $currency->id }}">
                        <td>{{ $currency->id }}</td>
                        <td>{{ $currency->name }}</td>
                        <td>{!! $currency->use_constant ?
                        ...
                        @canany(['view-currencies', 'update-currencies', 'delete-currencies', 'view-currencies-histories'])
                            ...
                            @can('update-currencies')
                                <a href="{{ $currency->editLink() }}"
                                   class="btn btn-sm btn-clean btn-icon btn-icon-md"
                                   title="{{ __('text.crud.edit') }}"
                                >
                                    <i class="fa fa-edit"></i>
                                </a>
                            @elsecan('view-currencies')
                                <a href="{{ $currency->editLink() }}"
                                   class="btn btn-sm btn-clean btn-icon btn-icon-md"
                                   title="{{ __('text.crud.edit') }}"
                                >
...
{{--                            @can('view-currencies-histories')--}}
{{--                                <a href="{{ $currency->transactionHistoriesLink() }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{ __('text.crud.edit') }}"><i class="fa fa-history"></i></a>--}}
{{--                            @endcan--}}

                            @can('delete-currencies')
                                <button class="btn btn-sm btn-clean btn-icon btn-icon-md"
                                        type="button"
                                        onclick="confirmToDelete('{{ $currency->deleteLink() }}', {{ $currency->id }})"
                                        title="{{ __('text.crud.delete') }}"
                                >
                                    <i class="fa fa-trash" id="spinner-button-{{ $currency->id }}"></i>
                                </button>
                            @endcan
                        </td>
                        @endcanany
