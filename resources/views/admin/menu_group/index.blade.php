...
@php
    ...
    $subHeaderBreadcrumbs = [['name' => __('text.menu_group.name_plural'), 'url' => route('admin.menu_groups.index'),'is_active' => true]];
@endphp
                    ...
                    <div class="btn-group show">
                        ...
                        <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(-52px, 38px, 0px);">
                            @can('create-menu-groups')
                                <a href="{{ route('admin.menu_groups.create') }}" class="dropdown-item" ><i class="fa fa-plus-circle"></i>{{ __('text.menu_group.add_new') }}</a>
                            @endcan

{{--                            @can('view-menu-groups-histories')--}}
{{--                                <a href="{{ route('admin.menu_groups.histories') }}" class="dropdown-item" ><i class="fa fa-history"></i>{{ trans_choice('text.common.transaction_history', 2) }}</a>--}}
{{--                            @endcan--}}
        ...
        {{ html()->form('GET', route('admin.menu_groups.index'))->class(['filter-form'])->open() }}
        ...
        <div class="kt-portlet__body">
                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group row">
                                <div class="col-lg-6">
                                    <label for="order" class="col-form-label">{{ __('text.common.row_number') }}</label>
                                    {{ html()->number('order')
                        ...
                        <div class="modal-footer">
                            <a href="{{ route('admin.menu_groups.index') }}" class="btn btn-danger"><i class="fa fa-eraser"></i>{{ __('text.crud.clear') }}</a>
            ...
            <table class="table table-striped- table-bordered table-hover table-checkable responsive no-wrap" id="datatable">
                ...
                <tr>
                    ...
                    <th>{{ __('text.common.row_number') }}</th>
                    <th class="status-column">{{ __('text.crud.status') }}</th>
                    @canany(['view-menu-groups', 'update-menu-groups', 'delete-menu-groups', 'view-menu-groups-histories'])
                ...
                @foreach($menuGroups as $menuGroup)
                    <tr class="table-row-id-{{ $menuGroup->id }}">
                        <td>{{ $menuGroup->id }}</td>
                        ...
                        @canany(['view-menu-groups', 'update-menu-groups', 'delete-menu-groups', 'view-menu-groups-histories'])
                            <td>
                                @can('update-menu-groups')
                                    <a href="{{ $menuGroup->editLink() }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{ __('text.crud.edit') }}"><i class="fa fa-edit"></i></a>
                                @elsecan('view-menu-groups')
                                    <a href="{{ $menuGroup->editLink() }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{ __('text.crud.edit') }}"><i class="fa fa-eye"></i></a>
                                @endcan

    {{--                            @can('view-menu-groups-histories')--}}
    {{--                                <a href="{{ $menuGroup->transactionHistoriesLink() }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{ __('text.crud.edit') }}"><i class="fa fa-history"></i></a>--}}
    {{--                            @endcan--}}

                                @can('delete-menu-groups')
                                    <button class="btn btn-sm btn-clean btn-icon btn-icon-md" type="button" onclick="confirmToDelete('{{ $menuGroup->deleteLink() }}', {{ $menuGroup->id }})" title="{{ __('text.crud.delete') }}">
                                        <i class="fa fa-trash" id="spinner-button-{{ $menuGroup->id }}"></i>
                                    </button>
                                @endcan
                            </td>
                        @endcanany
            ...
            {{ $menuGroups->appends(request()->all())->links() }}
