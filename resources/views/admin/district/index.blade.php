...
@php
    ...
    $subHeaderBreadcrumbs = [['name' => __('text.district.name_plural'), 'url' => route('admin.districts.index'),'is_active' => true]];
@endphp
                    ...
                    <div class="btn-group show">
                        ...
                        <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(-52px, 38px, 0px);">
                            @can('create-districts')
                                <a href="{{ route('admin.districts.create') }}" class="dropdown-item" ><i class="fa fa-plus-circle"></i>{{ __('text.district.add_new') }}</a>
                            @endcan

{{--                            @can('view-districts-histories')--}}
{{--                                <a href="{{ route('admin.districts.histories') }}" class="dropdown-item" ><i class="fa fa-history"></i>{{ trans_choice('text.common.transaction_history', 2) }}</a>--}}
{{--                            @endcan--}}
        ...
        {{ html()->form('GET', route('admin.districts.index'))->class(['filter-form'])->open() }}
        ...
        <div class="kt-portlet__body">
                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <a href="{{ route('admin.districts.index') }}" class="btn btn-danger"><i class="fa fa-eraser"></i>{{ __('text.crud.clear') }}</a>
            ...
            <table class="table table-striped- table-bordered table-hover table-checkable responsive no-wrap" id="datatable">
                ...
                <tr>
                    ...
                    <th>{{ __('text.province.name_singular') }}</th>
                    <th class="status-column">{{ __('text.crud.status') }}</th>
                    @canany(['view-districts', 'update-districts', 'delete-districts', 'view-districts-histories'])
                ...
                @foreach($districts as $district)
                    <tr class="table-row-id-{{ $district->id }}">
                        <td>{{ $district->id }}</td>
                        <td>{{ $district->translationForCurrentLocale->name }}</td>
                        <td>
                            <a href="{{ route('admin.provinces.edit', ['province' => $district->province->id]) }}"
                               hreflang="{{ app()->getLocale() }}"
                               title="{{ $district->province->translationForCurrentLocale->name }}">
                                {{ $district->province->translationForCurrentLocale->name }}
                            </a>
                        </td>
                        <td>
                            {!! $district->is_active
                        ...
                        @canany(['view-districts', 'update-districts', 'delete-districts', 'view-districts-histories'])
                            <td>
                                @can('update-districts')
                                    <a href="{{ $district->editLink() }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{ __('text.crud.edit') }}"><i class="fa fa-edit"></i></a>
                                @elsecan('view-districts')
                                    <a href="{{ $district->editLink() }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{ __('text.crud.edit') }}"><i class="fa fa-eye"></i></a>
                                @endcan

    {{--                            @can('view-districts-histories')--}}
    {{--                                <a href="{{ $district->transactionHistoriesLink() }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{ __('text.crud.edit') }}"><i class="fa fa-history"></i></a>--}}
    {{--                            @endcan--}}

                                @can('delete-districts')
                                    <button class="btn btn-sm btn-clean btn-icon btn-icon-md" type="button" onclick="confirmToDelete('{{ $district->deleteLink() }}', {{ $district->id }})" title="{{ __('text.crud.delete') }}">
                                        <i class="fa fa-trash" id="spinner-button-{{ $district->id }}"></i>
                                    </button>
                                @endcan
                            </td>
                        @endcanany
