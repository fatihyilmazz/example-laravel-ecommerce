...
@php
    ...
    $subHeaderBreadcrumbs = [['name' => __('text.brand.name_plural'), 'url' => route('admin.brands.index'),'is_active' => true]];
@endphp
                ...
                <div class="kt-portlet__head-actions">
                    <div class="btn-group show">
                        ...
                        <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(-52px, 38px, 0px);">
                            @can('create-brands')
                                <a href="{{ route('admin.brands.create') }}" class="dropdown-item" ><i class="fa fa-plus-circle"></i>{{ __('text.brand.add_new') }}</a>
                            @endcan

{{--                            @can('view-brands-histories')--}}
{{--                                <a href="{{ route('admin.brands.histories') }}" class="dropdown-item" ><i class="fa fa-history"></i>{{ trans_choice('text.common.transaction_history', 2) }}</a>--}}
{{--                            @endcan--}}
        ...
        {{ html()->form('GET', route('admin.brands.index'))->class(['filter-form'])->open() }}
        ...
        <div class="kt-portlet__body">
                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <a href="{{ route('admin.brands.index') }}" class="btn btn-danger"><i class="fa fa-eraser"></i>{{ __('text.crud.clear') }}</a>
            ...
            <table class="table table-striped- table-bordered table-hover table-checkable responsive no-wrap" id="datatable">
                ...
                <tr>
                    ...
                    @canany(['view-brands', 'update-brands', 'delete-brands', 'view-brands-histories'])
                ...
                @foreach($brands as $brand)
                    <tr class="table-row-id-{{ $brand->id }}">
                        <td>{{ $brand->id }}</td>
                        <td>{{ $brand->name }}</td>
                        <td>{{ $brand->order }}</td>
                        <td>
                        ...
                        @canany(['view-brands', 'update-brands', 'delete-brands', 'view-brands-histories'])
                            <td>
                                @can('update-brands')
                                    <a href="{{ $brand->editLink() }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{ __('text.crud.edit') }}"><i class="fa fa-edit"></i></a>
                                @elsecan('view-brands')
                                    <a href="{{ $brand->editLink() }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{ __('text.crud.edit') }}"><i class="fa fa-eye"></i></a>
                                @endcan

    {{--                            @can('view-brands-histories')--}}
    {{--                                <a href="{{ $brand->transactionHistoriesLink() }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{ __('text.crud.edit') }}"><i class="fa fa-history"></i></a>--}}
    {{--                            @endcan--}}

                                @can('delete-brands')
                                    <button class="btn btn-sm btn-clean btn-icon btn-icon-md" type="button" onclick="confirmToDelete('{{ $brand->deleteLink() }}', {{ $brand->id }})" title="{{ __('text.crud.delete') }}">
                                        <i class="fa fa-trash" id="spinner-button-{{ $brand->id }}"></i>
                                    </button>
                                @endcan
                            </td>
                        @endcanany
