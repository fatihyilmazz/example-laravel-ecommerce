...
@php
    $pageTitle = __("text.activity_log.management");

    $subHeaderTitle = __("text.activity_log.management");
    $subHeaderBreadcrumbs = [
        ['name' => trans_choice("text.activity_log.name",2), 'url' => route("admin.activity_log.index"),'is_active' => false],
    ];
@endphp
...
@section('content')
    ...
    <div class="kt-portlet kt-portlet--mobile">
        ...
        {{ html()->form('GET', route("admin.activity_log.index"))->class(['filter-form'])->open() }}
        ...
        <div class="kt-portlet__body">
                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="form-group row">
                                ...
                                <div class="col-4">
                                    ...
                                    {{ html()->select('description')
                                            ->class(['form-control select2 d-block w-100', 'is-invalid' => $errors->has('description')])
                            ...
                            <div class="form-group row">
                                <div class="col-lg-6">
                                    <label for="subject_type" class="col-form-label d-block">{{ __('text.common.modified_record') }}</label>
                                    {{ html()->select('subject_type')
                                        ->class(['form-control select2 d-block w-100', 'is-invalid' => $errors->has('subject_type')])
                                        ->options($subjectTypes)
                                        ->value((request()->get('subject_type') ? request()->get('subject_type') : old(('subject_type'))))
                                        ->attributes(['autocomplete' => 'off'])
                                        ->placeholder(__('text.crud.select'))
                                    }}

                                    @error('subject_type') <x-alert type="error" :message="$message"/> @enderror
                                </div>

                                <div class="col-lg-6">
                                    <label for="causer_id" class="col-form-label">{{ __('text.common.user') }}</label>
                                    {{ html()->select('causer_id')
                                        ->class(['form-control select2', 'is-invalid' => $errors->has('causer_id')])
                                        ->options($causers)
                        ...
                        <div class="modal-footer">
                            <a href="{{ route("admin.activity_log.index") }}" class="btn btn-danger"><i class="fa fa-eraser"></i>{{ __('text.crud.clear') }}</a>
            ...
            <table class="table table-striped- table-bordered table-hover table-checkable responsive no-wrap" id="datatable">
                ...
                @foreach($activities as $activity)
                    <tr class="table-row-id-{{ $activity->id }}">
                        <td>{{ $activity->id }}</td>
                        ...
                        <td>
                            <span>{{ $activity->subject->name ??
                                    $activity->subject->title ??
                                    $activity->subject->full_name ??
                                     $activity->subject->translationForCurrentLocale->name
                                  }}</span>

{{--                            @if(!empty($activity->subject->deleted_at))--}}
{{--                                <span>{{ $activity->subject->name }}</span>--}}
{{--                            @else--}}
{{--                                <a href="{{ route("admin.{$modelName}s.edit", ["{$modelName}" => $activity->subject->id]) }}">{{ $activity->subject->name }}</a>--}}
{{--                            @endif--}}
                        </td>
                        <td>
                            @if(!empty($activity->causer->deleted_at))
                                <span>{{ $activity->subject->name }}</span>
                            @else
                                <a href="{{ route('admin.users.edit', ['user' => $activity->causer->id]) }}">{{ $activity->causer->first_name. ' '. $activity->causer->last_name }}</a>
                            @endif

                        </td>
                        <td>{{ json_encode($activity->changes->get('old')) }}</td>
                        <td>{{ json_encode($activity->changes->get('attributes')) }}</td>
