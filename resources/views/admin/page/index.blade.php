...
@php
    ...
    $subHeaderBreadcrumbs = [['name' => __('text.page.name_plural'), 'url' => route('admin.pages.index'),'is_active' => true]];
@endphp
                ...
                <div class="kt-portlet__head-actions">
                    <div class="btn-group show">
                        ...
                        <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(-52px, 38px, 0px);">
                            @can('create-pages')
                                <a href="{{ route('admin.pages.create') }}" class="dropdown-item" ><i class="fa fa-plus-circle"></i>{{ __('text.page.add_new') }}</a>
                            @endcan

{{--                            @can('view-pages-histories')--}}
{{--                                <a href="{{ route('admin.pages.histories') }}" class="dropdown-item" ><i class="fa fa-history"></i>{{ trans_choice('text.common.transaction_history', 2) }}</a>--}}
{{--                            @endcan--}}
        ...
        {{ html()->form('GET', route('admin.pages.index'))->class(['filter-form'])->open() }}
        ...
        <div class="kt-portlet__body">
                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                            ...
                            <div class="form-group row">
                                <div class="col-12">
                                    ...
                                    {{ html()->select('is_published')
                                        ->class(['form-control', 'is-invalid' => $errors->has('is_active')])
                                        ->attributes(['autocomplete' => 'off'])
                        ...
                        <div class="modal-footer">
                            <a href="{{ route('admin.pages.index') }}" class="btn btn-danger"><i class="fa fa-eraser"></i>{{ __('text.crud.clear') }}</a>
            ...
            <table class="table table-striped- table-bordered table-hover table-checkable responsive no-wrap" id="datatable">
                ...
                <tr>
                    ...
                    @canany(['view-pages', 'update-pages', 'delete-pages', 'view-pages-histories'])
                ...
                @foreach($pages as $page)
                    <tr class="table-row-id-{{ $page->id }}">
                        ...
                        @canany(['view-pages', 'update-pages', 'delete-pages', 'view-pages-histories'])
                            <td>
                                @can('update-pages')
                                    <a href="{{ $page->editLink() }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{ __('text.crud.edit') }}"><i class="fa fa-edit"></i></a>
                                @elsecan('view-pages')
                                    <a href="{{ $page->editLink() }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{ __('text.crud.edit') }}"><i class="fa fa-eye"></i></a>
                                @endcan

    {{--                            @can('view-pages-histories')--}}
    {{--                                <a href="{{ $page->transactionHistoriesLink() }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{ __('text.crud.edit') }}"><i class="fa fa-history"></i></a>--}}
    {{--                            @endcan--}}

                                @can('delete-pages')
                                    <button class="btn btn-sm btn-clean btn-icon btn-icon-md" type="button" onclick="confirmToDelete('{{ $page->deleteLink() }}', {{ $page->id }})" title="{{ __('text.crud.delete') }}">
                                        <i class="fa fa-trash" id="spinner-button-{{ $page->id }}"></i>
                                    </button>
                                @endcan
                            </td>
                        @endcanany
