...
@php
    $pageTitle = __('text.admin.management');

    $subHeaderTitle = __('text.admin.management');
    $subHeaderBreadcrumbs = [['name' => __('text.admin.name_plural'), 'url' => route('admin.admins.index'),'is_active' => true]];
@endphp
                ...
                <div class="kt-portlet__head-actions">
                    <div class="btn-group show">
                        ...
                        <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(-52px, 38px, 0px);">
                            @can('create-admins')
                                <a href="{{ route('admin.admins.create') }}" class="dropdown-item" ><i class="fa fa-plus-circle"></i>{{ __('text.admin.add_new') }}</a>
                            @endcan

{{--                            @can('view-admins-histories')--}}
{{--                                <a href="{{ route('admin.admins.histories') }}" class="dropdown-item" ><i class="fa fa-history"></i>{{ trans_choice('text.common.transaction_history', 2) }}</a>--}}
{{--                            @endcan--}}
        ...
        {{ html()->form('GET', route('admin.admins.index'))->class(['filter-form'])->open() }}
        ...
        <div class="kt-portlet__body">
                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <a href="{{ route('admin.admins.index') }}" class="btn btn-danger"><i class="fa fa-eraser"></i>{{ __('text.crud.clear') }}</a>
            ...
            <table class="table table-striped- table-bordered table-hover table-checkable responsive no-wrap" id="datatable">
                ...
                <tr>
                    ...
                    @canany(['view-admins', 'update-admins', 'delete-admins', 'view-admins-histories'])
                ...
                @foreach($admins as $admin)
                    <tr class="table-row-id-{{ $admin->id }}">
                        <td>{{ $admin->id }}</td>
                        ...
                        @canany(['view-admins', 'update-admins', 'delete-admins', 'view-admins-histories'])
                            <td>
                                @can('update-admins')
                                    <a href="{{ $admin->editLink() }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{ __('text.crud.edit') }}"><i class="fa fa-edit"></i></a>
                                @elsecan('view-admins')
                                    <a href="{{ $admin->editLink() }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{ __('text.crud.edit') }}"><i class="fa fa-eye"></i></a>
                                @endcan

    {{--                            @can('view-admins-histories')--}}
    {{--                                <a href="{{ $admin->transactionHistoriesLink() }}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{ __('text.crud.edit') }}"><i class="fa fa-history"></i></a>--}}
    {{--                            @endcan--}}

                                @can('delete-admins')
                                    <button class="btn btn-sm btn-clean btn-icon btn-icon-md" type="button" onclick="confirmToDelete('{{ $admin->deleteLink() }}', {{ $admin->id }})" title="{{ __('text.crud.delete') }}">
                                        <i class="fa fa-trash" id="spinner-button-{{ $admin->id }}"></i>
                                    </button>
                                @endcan
                            </td>
                        @endcanany
